...
class dashboard_model extends CI_Model{  
    ...
    public function total_penjualan_bulan_ini_detail()
    {
      $data['ppn'] = $this->db->select('SUM(total) as total')->from('penjualan')->where('tanggal BETWEEN "'. date('Y-m-01'). '" and "'. date('Y-m-t').'" and jenis_penjualan = 1')->get()->row()->total;
      $data['nonppn'] = $this->db->select('SUM(total) as total')->from('penjualan')->where('tanggal BETWEEN "'. date('Y-m-01'). '" and "'. date('Y-m-t').'" and jenis_penjualan = 2')->get()->row()->total;
      $data['prekusor'] = $this->db->select('SUM(total) as total')->from('penjualan')->where('tanggal BETWEEN "'. date('Y-m-01'). '" and "'. date('Y-m-t').'" and jenis_penjualan = 3')->get()->row()->total;
      $data['oot'] = $this->db->select('SUM(total) as total')->from('penjualan')->where('tanggal BETWEEN "'. date('Y-m-01'). '" and "'. date('Y-m-t').'" and jenis_penjualan = 4')->get()->row()->total;
       return  $data;
    } 
