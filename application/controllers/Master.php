...
public function dataoperasional($resto)
{
    ...
    $list = $this->master_model->get_operasional_datatable($resto,$conditions);
    $data = array();
    foreach ($list as $r) {
        $row = array();
    ...
    $result = array(
        "draw" => $get['draw'],
        "recordsTotal" => $this->master_model->count_all_datatable_operasional($resto,$conditions),
        "recordsFiltered" => $this->master_model->count_filtered_datatable_operasional($resto,$conditions),
        "data" => $data,
    );
    echo json_encode($result);
}
...
public function operasionalpdf($value='')
{
        ...
        $this->load->view('member/laporan/laporan_operasional_pdf', $data);
}
}
